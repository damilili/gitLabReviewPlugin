...
public final class ToBranchPopupAction extends BasePopupAction {
    private final Project project;

    public ToBranchPopupAction(Project project, String filterName) {
        super(filterName);
    ...
    protected void createActions(Consumer<AnAction> actionConsumer) {
        actionConsumer.consume(new DumbAwareAction("All") {
            ...
            public void actionPerformed(AnActionEvent e) {
                RMListObservable.getInstance().filterToBranch("");
                updateFilterValueLabel("All");
            }
        });
        ...
        for (GitRepository gitRepository : repositories) {
            ...
            for (GitRemoteBranch remoteBranch : remoteBranches) {
                actionConsumer.consume(new DumbAwareAction(remoteBranch.getName()) {
                    @Override
                    public void actionPerformed(AnActionEvent e) {
                        RMListObservable.getInstance().filterToBranch(remoteBranch.getName());
                    }
                });
            }
        }
